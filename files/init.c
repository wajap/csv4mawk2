...
process_cmdline(int argc, char** argv)
{
   ...
   for (i = 1; i < argc && argv[i][0] == '-'; i = nextarg)
   {
	 ...
	 if (strcmp(argv[i], "--c=\\t") == 0 || strcmp(argv[i], "--csv=\\t") == 0) {
	    CSV = '\t';
	 } else {
	    int pos = 3;
	    CSV = ',';
	 ...
	 case 'v':
	    ...
	    if (strncmp(optarg,"FS=",3) == 0)
		FS_set = 1;
	    if (strncmp(optarg,"RS=",3) == 0)
		RS_set = 1;
	    break ;
...
/* #endif  */
   ...
   if (CSV) {
	char fs_shadow_csv = FS_SHADOW_CSV;
	if (FS_set) {
	    errmsg(0,"FS cannot be set when using --csv");
	    mawk_exit(2);
        }
	if (RS_set) {
	    errmsg(0,"RS cannot be set when using --csv");
	    mawk_exit(2);
        }
	cell_destroy(FS) ;
	FS->type = C_STRING ;
	FS->ptr =  new_STRING2(&fs_shadow_csv,1) ;
	cast_for_split(cellcpy(&fs_shadow, FS)) ;
	cell_destroy(FS) ;
	FS->type = C_STRING ;
	FS->ptr =  new_STRING2(" ",1) ;
   }
}
...
"\tmawk [-W option] [--csv[=char]] [-v var=value] [--] 'program text' file(s)",
"\tmawk [-W option] [--csv[=char]] [-v var=value] [-f program-file] [--] file(s)",
"",
"Generic options:",
"",
...
"\t               if \"\\035\" is used as third argument in split it will be csv splitting",
"\t               to have nornal spliting one has to use \"[\\035]\" as third argument in split",
"",
